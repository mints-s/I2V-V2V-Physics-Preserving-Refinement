...
Image-to-Video 생성 영상의 품질 향상을 위한 동역학 보존 제약 기반 출력 정제 헤드 연구용 저장소입니다. 이 프로젝트는 고정된 I2V 백본이 생성한 기준 영상의 움직임 구조를 동역학 선행정보로 보고, 그 후단에 학습 가능한 출력 정제 헤드인 DyRefHead를 결합해 시각 품질 개선과 움직임 보존 사이의 상충관계를 분석합니다.

## Overview

최근 Image-to-Video(I2V) 모델은 단일 이미지와 텍스트 조건으로 시간적 장면 변화를 생성할 수 있을 정도로 발전했습니다. 그러나 I2V 출력은 동작 단서를 포함하더라도 최종 영상의 선명도와 디테일이 충분하지 않을 수 있습니다. 별도 Video-to-Video(V2V) 정제 모델은 시각 품질을 높일 수 있지만, 강한 정제는 기준 영상의 움직임 구조를 바꿀 수 있습니다.

본 연구는 이 문제를 다루기 위해 다음 구조를 사용합니다.
...
DyRefHead는 기준 영상을 직접 대체하지 않고, 보정 잔차를 더하는 방식으로 정제 영상을 생성합니다.

```text
V_hat = clip(V0 + alpha * R_theta(V0), 0, 1)
```

여기서 `R_theta(V0)`는 DyRefHead가 예측한 보정 잔차이며, 본 실험에서는 `alpha = 0.5`를 사용했습니다.

학습 손실은 시각 품질 의사 목표와 동역학 보존 제약을 함께 사용합니다.
...
`Vp`는 VEnhancer n30 결과이며, 정답 영상이 아니라 시각적 디테일 개선 방향을 제공하는 의사 목표입니다.

DyRefHead v2의 기본 가중치는 다음과 같습니다.
...
DyRefHead v3는 edge 가중치를 높이고 temporal 가중치를 낮춘 절제 실험입니다. 이 설정은 시각 품질 개선과 움직임 크기 보존 사이의 상충관계를 확인하기 위해 사용했습니다.
...
Flow 기반 지표는 실제 물리 정합성을 직접 검증하는 지표가 아닙니다. 이 지표는 기준 I2V 출력 대비 움직임 이탈 정도를 확인하기 위한 보조 지표입니다.
...
VEnhancer n30은 가장 높은 VBench Imaging 값을 보였지만, mean Flow EPE와 Flow magnitude ratio도 크게 증가했습니다. 이는 강한 V2V 정제가 시각 품질을 높이는 동시에 기준 I2V 출력의 움직임 구조에서 더 많이 벗어날 수 있음을 보여줍니다.

DyRefHead v2는 VEnhancer n30만큼 강한 시각 품질 향상을 달성하지는 못했지만, Stage-1 I2V 출력보다 VBench Imaging을 소폭 향상시키면서 VEnhancer n30보다 낮은 움직임 이탈 보조 지표를 보였습니다. DyRefHead v3는 Flow magnitude ratio를 v2보다 더 1에 가깝게 만들었지만, VBench Imaging은 낮아졌습니다.
